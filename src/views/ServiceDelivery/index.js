...
const Dashboard = () => {
  ...
  const columns = [
    ...
    { field: 'srvc_service', headerName: 'Type', width: 130 },
  ...
  const handleEdit = id => {
    ...
    let endpoint = 'editServiceDelivery?table=fms_srvcdlvry&field=srvc_id&id=' + id
  ...
  const handleDelete = id => {
    Swal.fire({
      ...
      if (result.value) {
        ...
        let endpoint = `deleteStatus?table=fms_srvcdlvry&field=srvc_id&id=${id}&delete_status=srvc_status&value=1`
  ...
  function CustomToolbar () {
    return (
      <GridToolbarContainer >
      <h3 style={{fontSize:"1.285rem",fontWeight:"500"}}>Service Delivery</h3>
       <Box sx={{ flexGrow: 1 }} />
