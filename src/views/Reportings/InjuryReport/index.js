...
const Dashboard = ({ setShow, show }) => {
  ...
  const columns = [
    ...
    },   },
    {
      ...
      valueGetter: params => {
        //console.log(params)
        const date = new Date(params.row.injury_date)
    ...
    { field: 'injury_time', headerName: 'Time', width: 130 },
  ...
  useEffect(() => {
    ...
    let endpoint = `joinWithReportingList?table=fms_injury_report&status=0&company_id=${companyId}`;
  ...
  const handleEdit = async (id) => {
    try {
      const endpoint = `editReportingData?table=fms_injury_report&field=injury_id&id=${id}`
      let response = await COMMON_GET_FUN(BASE_URL, endpoint);
  ...
  const handleDelete = id => {
    Swal.fire({
      ...
      if (result.value) {
        let endpoint = `deleteStatus?table=fms_injury_report&field=injury_id&id=${id}&delete_status=status&value=1`
  ...
  function CustomToolbar () {
    return (
      <GridToolbarContainer >
      <h3 style={{fontSize:"1.285rem",fontWeight:"500"}}>Injury Reports</h3>
       <Box sx={{ flexGrow: 1 }} />
