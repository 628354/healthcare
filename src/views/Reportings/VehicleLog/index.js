...
const Dashboard = ({ setShow, show }) => {
  ...
  const columns = [
    ...
    },   },
    {
      ...
      valueGetter: params => {
        console.log(params)
        const date = new Date(params.row.vehicle_date)
    ...
    { field: 'total_km', headerName: 'Total K.M', width: 130 },
    { field: 'purpose_journey', headerName: 'Purpose', width: 130 },
  ...
  useEffect(() => {
    ...
    let endpoint = `joinWithReportingList?table=fms_vehicle_log&status=0&company_id=${companyId}`;
  ...
  const handleEdit = async (id) => {
    try {
      const endpoint = `editReportingData?table=fms_vehicle_log&field=vehicle_id&id=${id}`
      let response = await COMMON_GET_FUN(BASE_URL, endpoint);
  ...
  const handleDelete = id => {
    Swal.fire({
      ...
      if (result.value) {
        let endpoint = `deleteStatus?table=fms_vehicle_log&field=vehicle_id&id=${id}&delete_status=vehicle_status&value=1`
  ...
  function CustomToolbar () {
    return (
      <GridToolbarContainer >
      <h3 style={{fontSize:"1.285rem",fontWeight:"500"}}>Vehicle Logs</h3>
       <Box sx={{ flexGrow: 1 }} />
