...
const Dashboard = ({ setShow, show }) => {
  ...
  const columns = [
    ...
    { field: 'visit_time', headerName: 'Time', width: 130 },
    { field: 'visit_doctor_name', headerName: 'Doctor Name', width: 130 },
  ...
  useEffect(() => {
    ...
    let endpoint = `joinWithReportingList?table=fms_doctor_visit&status=0&company_id=${companyId}`;
  ...
  const handleEdit = async (id) => {
    try {
      const endpoint = `editReportingData?table=fms_doctor_visit&field=visit_id&id=${id}`
      let response = await COMMON_GET_FUN(BASE_URL, endpoint);
  ...
  const handleDelete = id => {
    Swal.fire({
      ...
      if (result.value) {
        let endpoint = `deleteStatus?table=fms_doctor_visit&field=visit_id&id=${id}&delete_status=visit_status&value=1`
  ...
  function CustomToolbar () {
    return (
      <GridToolbarContainer >
      <h3 style={{fontSize:"1.285rem",fontWeight:"500"}}>Doctor Visits</h3>
       <Box sx={{ flexGrow: 1 }} />
