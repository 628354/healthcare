...
const Dashboard = ({ setShow, show }) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      valueGetter: params => {
        console.log(params)
        const date = new Date(params.row.end_date)
        const day = date.getDate().toString().padStart(2, '0')
        const month = (date.getMonth() + 1).toString().padStart(2, '0') // Month is zero-based
        const year = date.getFullYear()
        // Concatenate in the "dd/mm/yyyy" format
    ...
    {
      field: `documentOn`,
      headerName: 'Document On',
      width: 180,
      valueGetter: params => {
        console.log(params)
        const date = new Date(params.row.documented_on)
        const day = date.getDate().toString().padStart(2, '0')
        const month = (date.getMonth() + 1).toString().padStart(2, '0') // Month is zero-based
        const year = date.getFullYear()
        // Concatenate in the "dd/mm/yyyy" format
  ...
  useEffect(() => {
    ...
    let endpoint = `joinWithReportingList?table=fms_progress_report&status=0&company_id=${companyId}`;
  ...
  const handleEdit = async (id) => {
    try {
      const endpoint = `editReportingData?table=fms_progress_report&field=report_id&id=${id}`
      let response = await COMMON_GET_FUN(BASE_URL, endpoint);
  ...
  const handleDelete = id => {
    Swal.fire({
      ...
      if (result.value) {
        let endpoint = `deleteStatus?table=fms_progress_report&field=report_id&id=${id}&delete_status=status&value=1`
  ...
  function CustomToolbar () {
    return (
      <GridToolbarContainer >
      <h3 style={{fontSize:"1.285rem",fontWeight:"500"}}>Progress Report</h3>
       <Box sx={{ flexGrow: 1 }} />
