...
const Dashboard = ({ setShow, show }) => {
  ...
  useEffect(() => {
    ...
    let endpoint = `joinWithReportingList?table=fms_restrictive_practice_logs&status=0&company_id=${companyId}`;
  ...
  const handleEdit = async (id) => {
    try {
      const endpoint = `editReportingData?table=fms_restrictive_practice_logs&field=practice_id&id=${id}`
      let response = await COMMON_GET_FUN(BASE_URL, endpoint);
  ...
  const handleDelete = id => {
    Swal.fire({
      ...
      if (result.value) {
        let endpoint = `deleteStatus?table=fms_restrictive_practice_logs&field=practice_id&id=${id}&delete_status=status&value=1`
  ...
  function CustomToolbar () {
    return (
      <GridToolbarContainer >
      <h3 style={{fontSize:"1.285rem",fontWeight:"500"}}>Restrictive Practice Logs</h3>
       <Box sx={{ flexGrow: 1 }} />
