...
const Dashboard = ({ setShow, show }) => {
  ...
  const columns = [
    ...
    { field: 'fedbck_name', headerName: 'Name', width: 130 },
  ...
  useEffect(() => {
    ...
    let endpoint = `joinWithReportingList?table=fms_feedback&status=0&company_id=${companyId}`;
  ...
  const handleEdit = async (id) => {
    try {
      const endpoint = `editReportingData?table=fms_feedback&field=fedbck_id&id=${id}`
      let response = await COMMON_GET_FUN(BASE_URL, endpoint);
  ...
  const handleDelete = id => {
    Swal.fire({
      ...
      if (result.value) {
        let endpoint = `deleteStatus?table=fms_feedback&field=fedbck_id&id=${id}&delete_status=fedbck_status&value=1`
  ...
  function CustomToolbar () {
    return (
      <GridToolbarContainer >
      <h3 style={{fontSize:"1.285rem",fontWeight:"500"}}>Feedback</h3>
       <Box sx={{ flexGrow: 1 }} />
