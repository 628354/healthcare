...
const Dashboard = ({ setShow, show }) => {
  ...
  useEffect(() => {
    ...
    let endpoint = `joinWithReportingList?table=fms_prn_balance&status=0&company_id=${companyId}`;
  ...
  const handleEdit = async (id) => {
    try {
      const endpoint = `getWhere?table=fms_prn_balance&field=balance_id&id=${id}`
      let response = await COMMON_GET_FUN(BASE_URL, endpoint);
  ...
  const handleDelete = id => {
    Swal.fire({
      ...
      if (result.value) {
        let endpoint = `deleteStatus?table=fms_prn_balance&field=balance_id&id=${id}&delete_status=status&value=1`
  ...
  function CustomToolbar () {
    return (
      <GridToolbarContainer >
      <h3 style={{fontSize:"1.285rem",fontWeight:"500"}}>PRN Balances</h3>
       <Box sx={{ flexGrow: 1 }} />
