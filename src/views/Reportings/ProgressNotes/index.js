...
const Dashboard = ({ setShow, show,divShadow,participantId }) => {
  ...
  const columns = [
    ...
    { field: 'prgs_strttime', headerName: 'Shift start time', width: 130 },
    { field: 'prgs_endtime', headerName: 'Shift end time', width: 130 },
  ...
  useEffect(() => {
    ...
    let endpoint = `joinWithReportingList?table=fms_prtcpnt_prgsnote&status=0&company_id=${companyId}`;
  ...
  const handleEdit = async (id) => {
    try {
      const endpoint = `editReportingData?table=fms_prtcpnt_prgsnote&field=prgs_id&id=${id}`
      let response = await COMMON_GET_FUN(BASE_URL, endpoint);
  ...
  const handleDelete = id => {
    Swal.fire({
      ...
      if (result.value) {
        let endpoint = `deleteStatus?table=fms_prtcpnt_prgsnote&field=prgs_id&id=${id}&delete_status=prgs_status&value=1`
  ...
  const settingPage=()=>{
    ...
    navigate('/progress-notes/settings')
  }
  ...
  function CustomToolbar () {
    return (
      <GridToolbarContainer >
      <h3 style={{fontSize:"1.285rem",fontWeight:"500"}}>Shift Progress Notes</h3>
       <Box sx={{ flexGrow: 1 }} />
