...
const Dashboard = ({ setShow, show }) => {
  ...
  const columns = [
    ...
    { field: 'plc_name', headerName: 'Name', width: 130 },
    
    { field:`name`, headerName: 'Next review date', width: 150,
    renderCell: (params)=>{
      //console.log(params);
      
      if (params.row.plc_rvudate ==='0000-00-00') {
        return <div className='commonCla grayClr'>No date</div>
      } else {
        const date = new Date(params.row.plc_rvudate);
    ...
    { field: 'plc_version', headerName: 'Version', width: 130 },
  ...
  const handleEdit = id => {
    try {
      let endpoint = 'editCompanyData?table=fms_policies&field=plc_id&id=' + id
  ...
  const handleDelete = id => {
    Swal.fire({
      ...
      if (result.value) {
        let endpoint = `deleteStatus?table=fms_policies&field=plc_id&id=${id}&delete_status=pls_status&value=1`
        let response = COMMON_GET_FUN(BASE_URL, endpoint)
        response.then(data => {
          if (data.status) {
            Swal.fire({
  ...
  function CustomToolbar () {
    return (
      <GridToolbarContainer >
      <h3 style={{fontSize:"1.285rem",fontWeight:"500"}}>Policies</h3>
       <Box sx={{ flexGrow: 1 }} />
