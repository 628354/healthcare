...
const Dashboard = ({ setShow, show }) => {
  ...
  const columns = [
    ...
    { field: 'rp_name', headerName: 'Name', width: 130 },
    
   
  

    { field: 'rp_vrsn', headerName: 'Version', width: 130 },
  ...
  useEffect(() => {
    try {
      let endpoint = `joinWithCompanyList?table=fms_rpdhsresources&status=0&company_id=${companyId}`;
  ...
  const handleEdit = id => {
    try {
      let endpoint = 'editCompanyData?table=fms_rpdhsresources&field=rp_id&id=' + id
  ...
  const handleDelete = id => {
    Swal.fire({
      ...
      if (result.value) {
        ...
        let endpoint = `deleteStatus?table=fms_rpdhsresources&field=rp_id&id=${id}&delete_status=rp_status&value=1`
        let response = COMMON_GET_FUN(BASE_URL, endpoint)
        response.then(data => {
          if (data.status) {
            Swal.fire({
  ...
  function CustomToolbar () {
    return (
      <GridToolbarContainer >
      <h3 style={{fontSize:"1.285rem",fontWeight:"500"}}>RP DHS Resources</h3>
       <Box sx={{ flexGrow: 1 }} />
