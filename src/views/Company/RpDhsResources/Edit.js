...
const Edit = ({ selectedData, setIsEditing, allowPre, setShow }) => {
  ...
  const currentDate = new Date();
  const [name, setName] = useState(selectedData.rp_name);
  const [version, setVersion] = useState(selectedData.rp_vrsn)
  const [nextReviewDate, setNextReviewDate] = useState(selectedData.rp_rvudate === "0000-00-00" ? "" : selectedData.rp_rvudate)
  ...
  const handleUpdate = (e) => {
    ...
    formData.append('rp_rvudate', newdateFormat);
