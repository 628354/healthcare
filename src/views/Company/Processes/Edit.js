...
import { BASE_URL, COMMON_UPDATE_FUN, IMG_BASE_URL,COMMON_GET_FUN  } from '../../../helper/ApiInfo'
//select field
...
const Edit = ({ selectedData, setIsEditing, allowPre, setShow }) => {
  ...
  const id = selectedData.prs_id;
  const currentDate = new Date();
  const [name, setName] = useState(selectedData.prs_name);
  const [version, setVersion] = useState(selectedData.prs_versn)
  const [nextReviewDate, setNextReviewDate] = useState(selectedData.prs_rvudate === "0000-00-00" ? "" : selectedData.prs_rvudate)
  ...
  const handleUpdate = (e) => {
    ...
    formData.append('prs_name', name);
    formData.append('prs_versn', version);
    formData.append('prs_rvudate', newdateFormat);
    ...
    let endpoint = 'updateComapny?table=fms_Processes&field=prs_id&id=' + id;
