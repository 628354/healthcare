...
const Dashboard = ({ setShow, show }) => {
  ...
  const columns = [
    ...
    { field: 'prs_versn', headerName: 'Version', width: 130 },
  ...
  const handleEdit = id => {
    try {
      let endpoint = 'editCompanyData?table=fms_Processes&field=prs_id&id=' + id
  ...
  const handleDelete = id => {
    Swal.fire({
      ...
      if (result.value) {
        let endpoint = `deleteStatus?table=fms_Processes&field=prs_id&id=${id}&delete_status=prs_status&value=1`
        let response = COMMON_GET_FUN(BASE_URL, endpoint)
        response.then(data => {
          if (data.status) {
            Swal.fire({
  ...
  function CustomToolbar () {
    return (
      <GridToolbarContainer >
      <h3 style={{fontSize:"1.285rem",fontWeight:"500"}}>Processes</h3>
       <Box sx={{ flexGrow: 1 }} />
