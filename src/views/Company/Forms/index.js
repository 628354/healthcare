...
const Dashboard = ({ setShow, show,selectedTab }) => {
  ...
  const columns = [
    ...
    { field: 'form_name', headerName: 'Name', width: 130 },
    { field: 'form_version', headerName: 'Version', width: 130 },
    { field:`form_type`, headerName: 'Type ', width: 130, },
...
useEffect(() => {
  try {
    let endpoint = `joinWithCompanyList?table=fms_form&status=0&company_id=${companyId}&value=${selectedTab}`;
...
const handleEdit = id => {
  try {
    let endpoint = 'editCompanyData?table=fms_form&field=form_id&id=' + id
  ...
  const handleDelete = id => {
    Swal.fire({
      ...
      if (result.value) {
        let endpoint = `deleteStatus?table=fms_form&field=form_id&id=${id}&delete_status=form_status&value=1`
        let response = COMMON_GET_FUN(BASE_URL, endpoint)
        response.then(data => {
          if (data.status) {
            Swal.fire({
  ...
  function CustomToolbar () {
    return (
      <GridToolbarContainer >
      <h3 style={{fontSize:"1.285rem",fontWeight:"500"}}>{selectedTab} Forms</h3>
       <Box sx={{ flexGrow: 1 }} />
