...
const Dashboard = ({ setShow, show }) => {
  ...
  const columns = [
    ...
    { field:`name`, headerName: 'Date', width: 180,
                    ...
                    { field: 'call_dscrptn', headerName: 'Description', width: 160 },
...
<DeleteOutlineOutlinedIcon />
  ...
  const handleEdit = id => {
    try {
      const endpoint = `getWhere?table=fms_call_log&field=call_id&id=${id}`;
  ...
  const handleDelete = id => {
    Swal.fire({
      ...
      if (result.value) {
        let endpoint = `deleteStatus?table=fms_call_log&field=call_id&id=${id}&delete_status=call_status&value=1`
        let response = COMMON_GET_FUN(BASE_URL, endpoint)
        response.then(data => {
          if (data.status) {
            Swal.fire({
  ...
  const CustomToolbar = () => (
    ...
    <h3 style={{fontSize:"1.285rem",fontWeight:"500"}}>On Call Logs</h3>
     <Box sx={{ flexGrow: 1 }} />
...
className={employees.length<1?"hide_tableData":""}
      ...
      {isEditing && <Edit selectCallLog={selectedDocument}  setShow={setShow} setIsEditing={setIsEditing} allowPre={allowPre}/>}
    </div>
  );
};
