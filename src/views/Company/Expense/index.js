...
const Dashboard = ({ setShow, show }) => {
  ...
  const columns = [
    {
      ...
      valueGetter: params => {
        console.log(params)
        const date = new Date(params.row.expn_date)
    ...
    { field: 'expn_totlexpn', headerName: 'Total Expense', width: 130 },
    { field: 'expn_paid', headerName: 'Paid By', width: 130 },
    { field:`expense_status`, headerName: 'Status ', width: 130, 
    renderCell: (params)=>{
      
      if (params.row.expense_status ==='Pending') {
        return <div className='commonCla orangeClr'>{params.row.expense_status}</div>
      } else if (params.row.expense_status ==='Approved') {
        return <div className='commonCla greenClr'>{params.row.expense_status}</div>
        
      } else {
        return <div className='commonCla redClr' >{params.row.expense_status}</div>
      }
  ...
  useEffect(() => {
    try {
      ...
      let endpoint = `joinWithCompanyList?table=fms_expenses&status=0&company_id=${companyId}`;
  ...
  const handleEdit = id => {
    try {
      let endpoint = 'editCompanyData?table=fms_expenses&field=expn_id&id=' + id
  ...
  const handleDelete = id => {
    Swal.fire({
      ...
      if (result.value) {
        let endpoint = `deleteStatus?table=fms_expenses&field=expn_id&id=${id}&delete_status=status&value=1`
        let response = COMMON_GET_FUN(BASE_URL, endpoint)
        response.then(data => {
          if (data.status) {
            Swal.fire({
  ...
  function CustomToolbar () {
    return (
      <GridToolbarContainer >
      <h3 style={{fontSize:"1.285rem",fontWeight:"500"}}>Expenses</h3>
       <Box sx={{ flexGrow: 1 }} />
