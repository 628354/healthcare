...
const Edit = ({ selectedData, setIsEditing, allowPre, setShow }) => {
  ...
  const id = selectedData.prac_id;
  const currentDate = new Date();
  const [name, setName] = useState(selectedData.prac_name);
  const [version, setVersion] = useState(selectedData.prac_version)
  const [nextReviewDate, setNextReviewDate] = useState(selectedData.prac_rvudate === "0000-00-00" ? "" : selectedData.prac_rvudate)
  ...
  const handleUpdate = (e) => {
    ...
    formData.append('prac_name', name);
    formData.append('prac_version', version);
    formData.append('prac_rvudate', newdateFormat);
    ...
    let endpoint = 'updateComapny?table=fms_practiceguide&field=prac_id&id=' + id;
