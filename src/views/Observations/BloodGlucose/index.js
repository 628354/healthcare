...
const Dashboard = ({ setShow, show }) => {
  ...
  const columns = [
...

    {
      ...
      valueGetter: (params) => {
        console.log(params);
        const date = new Date(params.row.glucose_date);
    ...
    { field: 'glucose_read', headerName: 'Reading (mg/dL)', width: 160 },
...
<VisibilityIcon />
  ...
  useEffect(() => {
    ...
    let endpoint = `joinWithObservations?table=fms_obsglucose&status=0&company_id=${companyId}`;
  ...
  const handleEdit = async (id) => {
    try {
      const endpoint = `getWhere?table=fms_obsglucose&field=glucose_id&id=${id}`
      let response = await COMMON_GET_FUN(BASE_URL, endpoint);
  ...
  const handleDelete = id => {
    Swal.fire({
      ...
      if (result.value) {
        let endpoint = `deleteStatus?table=fms_obsglucose&field=glucose_id&id=${id}&delete_status=glucose_status&value=1`
  ...
  const CustomToolbar = () => (
    <GridToolbarContainer >
      <h3 style={{ fontSize: "1.285rem", fontWeight: "500" }}>Blood Glucose Logs <span><InfoIcon onClick={handleCardOpen} /></span></h3>
      <Box sx={{ flexGrow: 1 }} />
      ...
      }
      {
        showInfo ? <Card sx={{ width: '100%' }} className='ObCard' >
          ...
          <div className='headerCard'>
            ...
            <CardContent>
              <CardHeader
                title="Introduction to Blood Glucose Logs"
              // subheader="September 14, 2016"
              />
              <Typography variant="body2" color="text.secondary">
                Care Diary allows you to record Blood Glucose Logs for your participants and visualize this data in the form of charts. You can also export this data in CSV or PDF format.
              </Typography>
            </CardContent>
