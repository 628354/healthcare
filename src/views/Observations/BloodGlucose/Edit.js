...
const Edit = ({ selectData, setIsEditing, allowPre, setShow }) => {
  ...
  const id = selectData.glucose_id;
  const [date, setDate] = useState(selectData.glucose_date ? dayjs(selectData.glucose_date) : null);
  const [time, setTime] = useState(selectData.glucose_time);
  const [staff, setStaff] = useState(selectData.glucose_stfid)
  const [participant, setParticipant] = useState(selectData.glucose_prtcpntid);
  const [participantList, setParticipantList] = useState([])
  const [staffList, setStaffList] = useState([])
  const [reading, setReading] = useState(selectData.glucose_read);
  const [comments, setComments] = useState(selectData.glucose_cmnt);
  ...
  const handleUpdate = e => {
    ...
    formData.append('glucose_date', dateFormat)
    formData.append('glucose_time', formattedTime)
    formData.append('glucose_stfid', staff)
    formData.append('glucose_prtcpntid', participant)
    formData.append('glucose_read', reading)
    formData.append('glucose_cmnt', comments)
    ...
    let endpoint = 'updateAll?table=fms_obsglucose&field=glucose_id&id=' + id
