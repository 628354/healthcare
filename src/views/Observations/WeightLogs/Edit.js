...
const Edit = ({ selectData, setIsEditing,allowPre ,setShow}) => {
  ...
  const id = selectData.wgt_id;
  const [date, setDate] = useState(selectData.wgt_date? dayjs(selectData.wgt_date): null);
  const [time, setTime] = useState(selectData.wgt_time? dayjs(selectData.wgt_time, 'HH:mm') : null);
  const[staff,setStaff]=useState(selectData.wgt_stfid)
  const [participant, setParticipant] = useState(selectData.wgt_prtcpntid);
  const [participantList,setParticipantList]=useState([])
  const[staffList,setStaffList]=useState([])
  const [reading, setReading] = useState(selectData.wgt_read);
  const [comments, setComments] = useState(selectData.wgt_cmnts);
  ...
  const handleUpdate = e => {
    ...
    formData.append('wgt_date', dateFormat)
    formData.append('wgt_time', formattedTime)
    formData.append('wgt_stfid', staff)
    formData.append('wgt_prtcpntid', participant)
    formData.append('wgt_read', reading)
    formData.append('wgt_cmnts', comments)
    formData.append('updated_at', currentTime)


    let endpoint = 'updateAll?table=fms_weight&field=wgt_id&id=' + id
