...
const Dashboard = ({ setShow, show }) => {
  ...
  const columns = [
    ...
    { field:`name`, headerName: 'Date', width: 120,
                    valueGetter: (params)=>{
                      console.log(params);
                        const date = new Date(params.row.bwl_date);
                    ...
                    { field: 'bwl_time', headerName: 'Time', width: 100 },
                    { field: 'bwl_status', headerName: 'Status', width: 120 },
                    { field: 'bwl_type', headerName: 'Type', width: 120 },
...
<DeleteOutlineOutlinedIcon />
  ...
  useEffect(() => {
    ...
    let endpoint = `joinWithObservations?table=fms_bowel&status=0&company_id=${companyId}`;
  ...
  const handleEdit = async (id) => {
    try {
      const endpoint = `getWhere?table=fms_bowel&field=bwl_id&id=${id}`
      let response = await COMMON_GET_FUN(BASE_URL, endpoint);
  ...
  const handleDelete = id => {
    Swal.fire({
      ...
      if (result.value) {
        let endpoint = `deleteStatus?table=fms_bowel&field=bwl_id&id=${id}&delete_status=status&value=1`
  ...
  const CustomToolbar = () => (
    ...
    <h3 style={{fontSize:"1.285rem",fontWeight:"500"}}>Bowel Logs<span><InfoIcon   onClick={handleCardOpen}/></span></h3>
     <Box sx={{ flexGrow: 1 }} />
                              ...
                              }
{
  ...
  <IconButton aria-label="settings" className='cardIcon'>
        ...
        <CardContent>
        <CardHeader
          title="Introduction to Bowel Logs"
          // subheader="September 14, 2016"
        />
          <Typography variant="body2" color="text.secondary">
          Care Diary allows you to record Bowel Logs for your participants. You can also export this data in CSV or PDF format.
          </Typography>
        </CardContent>
