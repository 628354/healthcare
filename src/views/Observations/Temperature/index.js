...
const Dashboard = ({ setShow, show }) => {
  ...
  const columns = [
    ...
    { field:`name`, headerName: 'Date', width: 180,
                    valueGetter: (params)=>{
                      console.log(params);
                        const date = new Date(params.row.temp_date);
                    ...
                    { field: 'temp_readng', headerName: 'Reading (℉)', width: 160 },
...
<DeleteOutlineOutlinedIcon />
  ...
  useEffect(() => {
    let endpoint = `joinWithObservations?table=fms_temp&status=0&company_id=${companyId}`;
  ...
  const handleEdit = async (id) => {
    try {
      const endpoint = `getWhere?table=fms_temp&field=temp_id&id=${id}`
      let response = await COMMON_GET_FUN(BASE_URL, endpoint);
  ...
  const handleDelete = id => {
    Swal.fire({
      ...
      if (result.value) {
        let endpoint = `deleteStatus?table=fms_temp&field=temp_id&id=${id}&delete_status=temp_status&value=1`
  ...
  const CustomToolbar = () => (
    ...
    <h3 style={{fontSize:"1.285rem",fontWeight:"500"}}>Temperature Logs <span><InfoIcon   onClick={handleCardOpen}/></span></h3>
     <Box sx={{ flexGrow: 1 }} />
                              ...
                              }
{
  ...
  <IconButton aria-label="settings" className='cardIcon'>
        ...
        <CardContent>
        <CardHeader
          title="Introduction to Temperature Logs"
          // subheader="September 14, 2016"
        />
          <Typography variant="body2" color="text.secondary">
          Care Diary allows you to record Temperature Logs for your participants and visualize this data in the form of charts. You can also export this data in CSV or PDF format.
          </Typography>
        </CardContent>
