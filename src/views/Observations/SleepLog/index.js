...
const Dashboard = ({ setShow, show }) => {
  ...
  const columns = [
    ...
    { field: 'slp_start_time', headerName: 'Time', width: 120 },
    { field: 'slp_activity', headerName: 'Activity', width: 120 },
    {
...
<VisibilityIcon />
  ...
  useEffect(() => {
    ...
    let endpoint = `joinWithObservations?table=fms_slplog&status=0&company_id=${companyId}`;
  ...
  const handleEdit = async (id) => {
    try {
      const endpoint = `getWhere?table=fms_slplog&field=slp_id&id=${id}`
      let response = await COMMON_GET_FUN(BASE_URL, endpoint);
  ...
  const handleDelete = id => {
    Swal.fire({
      ...
      if (result.value) {
        let endpoint = `deleteStatus?table=fms_slplog&field=slp_id&id=${id}&delete_status=slp_status&value=1`
  ...
  const CustomToolbar = () => (
    <GridToolbarContainer >
      <h3 style={{ fontSize: "1.285rem", fontWeight: "500" }}>Sleep Logs<span><InfoIcon onClick={handleCardOpen} /></span></h3>
      <Box sx={{ flexGrow: 1 }} />
