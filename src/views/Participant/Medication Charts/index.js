...
const Dashboard = ({setShow, show}) => {
  ...
  const columns = [
    ...
    { field:`name`, headerName: 'Date', width: 180,
                    ...
                    { field: 'mdi_note', headerName: 'Notes', width: 160 },
                    { field: 'mdi_stf', headerName: 'Staff', width: 160 },
    {
      field: 'action',
      headerName: 'Action',
      width: 250,
...
<DeleteOutlineOutlinedIcon />
  ...
  const handleEdit = id => {
    try {
      const endpoint = `editParticipant?table=fms_medicationchart&field=mdi_id&id=${id}`;
  ...
  const handleDelete = id => {
    Swal.fire({
      ...
      if (result.value) {
        let endpoint = `deleteStatus?table=fms_medicationchart&field=mdi_id&id=${id}&delete_status=mdi_status&value=1`
        let response = COMMON_GET_FUN(BASE_URL, endpoint)
        response.then(data => {
          if (data.status) {
            Swal.fire({
  ...
  const CustomToolbar = () => (
    ...
    <h3 style={{fontSize:"1.285rem",fontWeight:"500"}}>Medication Chart</h3>
     <Box sx={{ flexGrow: 1 }} />
  ...
  return (
    <div className="container">
      ...
      {isEditing && <Edit selectedMadication={selectedDocument} setShow={setShow} show={show} setIsEditing={setIsEditing} allowPre={allowPre}/>}
    </div>
  );
};
