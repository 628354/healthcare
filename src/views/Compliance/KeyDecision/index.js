...
const Dashboard = ({ setShow, show }) => {
  ...
  const columns = [
    ...
    { field: 'key_dsrptn', headerName: 'Description', width: 130 },
   
    { field: 'key_ratnle', headerName: 'Decision Rationale ', width: 130 },
  ...
  const handleDelete = id => {
    Swal.fire({
      ...
      if (result.value) {
        ...
        let endpoint = `deleteStatus?table=fms_key_decision&field=key_id&id=${id}&delete_status=key_status&value=1`
        let response = COMMON_GET_FUN(BASE_URL, endpoint)
        response.then(data => {
          if (data.status) {
            Swal.fire({
  ...
  function CustomToolbar () {
    return (
      <GridToolbarContainer >
      <h3 style={{fontSize:"1.285rem",fontWeight:"500"}}>Key Decision</h3>
       <Box sx={{ flexGrow: 1 }} />
