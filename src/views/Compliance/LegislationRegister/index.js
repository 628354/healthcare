...
const Dashboard = ({ setShow, show }) => {
  ...
  const columns = [
    {
      ...
      valueGetter: params => {
        console.log(params)
        const date = new Date(params.row.legis_rvuon)
    ...
    { field: `legis_domain`, headerName: 'Domain', width: 130, },

    { field: 'regis_refrnc', headerName: 'Legislative reference', width: 130 },
  ...
  useEffect(() => {
    let endpoint = `joinWithComplianceList?table=fms_legislation_registers&status=0&company_id=${companyId}`;
  ...
  const handleEdit = id => {
    let endpoint = 'editComplianceData?table=fms_legislation_registers&field=legis_id&id=' + id
    let response = COMMON_GET_FUN(BASE_URL, endpoint)
    response.then(data => {
  ...
  const handleDelete = id => {
    Swal.fire({
      ...
      if (result.value) {
        let endpoint = `deleteStatus?table=fms_legislation_registers&field=legis_id&id=${id}&delete_status=regis_status&value=1`
  ...
  function CustomToolbar() {
    return (
      <GridToolbarContainer >
        <h3 style={{ fontSize: "1.285rem", fontWeight: "500" }}>Legislation Registers</h3>
        <Box sx={{ flexGrow: 1 }} />
